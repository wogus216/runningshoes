...
const STRIP_WIDTH = 1920 / MEMBERS.length;

// 16:9 — 7개 세로 스트립이 화면 전체 폭을 균등 분할. 릴과 같은 순서·클립 리듬.
export const AssembleWebScene: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.paper, color: COLORS.ink }}>
          ...
          >
            <Img
              ...
              style={{
                ...
                top: -40,
                bottom: -40,
                left: `${-index * (1920 / 7 - STRIP_WIDTH / 3.4)}px`,
                width: 1920,
                height: 1160,
                objectFit: "cover",
      ...
      >
        <div style={{ fontFamily: FONT, fontSize: 92, fontWeight: 900, lineHeight: 0.96, letterSpacing: "-0.06em" }}>
          {SATURDAY_COPY.crew.title}
        </div>
        <div style={{ color: COLORS.signalDark, fontFamily: MONO, fontSize: 22, fontWeight: 600, textAlign: "right", lineHeight: 1.7 }}>
          {DISTANCE_TOTALS[0]}
          <br />{DISTANCE_TOTALS[1]}
          <br />{MEMBERS.length} × START LINE
        </div>
      </div>
