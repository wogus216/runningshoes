...
**Interfaces:**
- Produces: 기준선 문서. Task 8(성과 측정)이 이 수치와 대조한다.
...
### Task 1: L2 — 코스 9편 제목·요약 수리

가장 중요한 변경이다. `달리기`를 제목 앞쪽에 넣고 `러닝 코스`를 병기한다.

**Files:**
- Modify: `src/lib/data/blog/posts/2026-05.ts` (8편)
...
9편 각각의 `tags` 배열에 `'{도시} 달리기'`와 `'{도시} 러닝코스'`(띄어쓰기 없는 형태)를 추가한다. 기존 태그는 지우지 않는다.
...
Run:
```bash
grep -c "달리기 좋은 곳 7선" src/lib/data/blog/posts/2026-05.ts src/lib/data/blog/posts/2026-06.ts
```
Expected: `2026-05.ts:8` / `2026-06.ts:1` (합계 9)

Run:
```bash
grep -c "이색 러닝 코스 7선" src/lib/data/blog/posts/2026-05.ts src/lib/data/blog/posts/2026-06.ts
```
Expected: 둘 다 `0` — 남아 있으면 교체 누락이다
...
```
Expected: 2026-05.ts는 8 이상, 2026-06.ts는 1 이상. **slug는 절대 바뀌면 안 된다.**
...
제목만 바꾸면 본문과 어긋난다. 도입부 첫 문단에도 검색어를 넣고, 고립돼 있던 글끼리 연결한다.
...
- [ ] **Step 2: 도시 간 상호 링크 추가**

각 글 마지막에 아래 블록을 추가한다. **자기 자신은 제외**하고 나머지 8개를 나열한다. 이 시리즈는 지금 서로 연결돼 있지 않아 고립 상태다.

예(수원 글에 넣을 블록 — 수원은 빠져 있음):
```html
<h2>다른 도시에서 달릴 곳</h2>
<ul>
  <li><a href="/blog/busan-unique-running-courses-7">부산 달리기 좋은 곳 7선</a></li>
  <li><a href="/blog/incheon-unique-running-courses-7">인천 달리기 좋은 곳 7선</a></li>
  <li><a href="/blog/daegu-unique-running-courses-7">대구 달리기 좋은 곳 7선</a></li>
  <li><a href="/blog/daejeon-unique-running-courses-7">대전 달리기 좋은 곳 7선</a></li>
  <li><a href="/blog/gwangju-unique-running-courses-7">광주 달리기 좋은 곳 7선</a></li>
  <li><a href="/blog/ulsan-unique-running-courses-7">울산 달리기 좋은 곳 7선</a></li>
  <li><a href="/blog/jeju-unique-running-courses-7">제주 달리기 좋은 곳 7선</a></li>
  <li><a href="/blog/chuncheon-unique-running-courses-7">춘천 달리기 좋은 곳 7선</a></li>
</ul>
```

**노출 0인 4편(서울·전주·경주·강릉)은 링크하지 않는다** — 수리 대상이 아니고, 링크 가치를 그쪽으로 흘릴 이유가 없다.

- [ ] **Step 3: `updatedAt` 갱신**

9편 각각에 `updatedAt: '2026-08-18'`을 추가한다(이미 있으면 갱신). `publishedAt`은 **건드리지 않는다**.
