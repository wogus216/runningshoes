...
`/blog`에서 "더 보기"를 5회 클릭해 카드 144장을 로드한 뒤(BLOG_PAGE_SIZE=24 페이지네이션,
`asics-gel-kayano-33-review`는 134번째 카드 — 121~144번째 구간에서 로드됨. 클릭마다
`findIndex`로 위치를 확인해 정확한 클릭 횟수를 잡았다), 해당 카드를 360px 폭에서
스크린샷했다.

**추가 정정 (같은 날, 킥커 텍스트 제거 결정 반영)**: 운영자가 재측정 직후 킥커 텍스트
자체를 없애기로 결정했다 — 카드 하단에 "리뷰 · 2026년 6월 5일"이 바로 이어지고 상세
페이지 헤더도 같은 카테고리를 또 반복해, 썸네일 안 킥커 텍스트("— 리뷰")가 중복이었다.
`scripts/thumbnail/copy.ts`의 `deriveCopy`가 이제 `kicker: ''`을 고정 반환하고(카테고리
자동 파생 제거), 33장을 재생성했다. 아래는 이 변경 반영 후 재측정 결과다.

- 오렌지 대시(카테고리 텍스트 없이 대시만) · 제목("아식스 젤 카야노 33 솔직 리뷰") ·
  서브타이틀("13년 만의 구조 대변혁") · 데이터 줄("298g · 힐 36.8mm · SA 132") ·
  출처줄("allrunabout.com")까지 전부 360px 폭에서 잘리지 않고 읽힌다. 대시만 남아도
  제목과의 간격이 어색하지 않다(`.kicker` `margin-bottom: 30px` 그대로 유지).
- 좌표 실측(§0 스니펫과 동일 방식): `natural 1200×630(1.905)` vs `box 326×170.15(1.916)`,
  `cropPct -0.585%`(서브픽셀 반올림, §1의 다른 카드 -0.5%와 같은 성격) — 스펙 허용오차
  ±0.5%를 근소하게 벗어나지만 스크린샷상 시각적 잘림은 없다. 이미지 파일 자체(1200×630)는
  킥커 텍스트 제거 전후로 크기·비율이 동일해 이 수치는 변하지 않는다.
...
- 밀도를 3단(대시/제목/데이터 한 줄)으로 줄인 목적대로, 정보가 뭉개지지 않고 스캔 가능한
  크기로 렌더된다.
