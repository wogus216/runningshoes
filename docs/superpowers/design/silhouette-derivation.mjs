...
// 뷰박스 0 0 560 400.
//
// 배치 기준 — **구현자 재량 판단** (2026-07-31). 운영자·controller 가 승인한 스펙이 아니다.
// "x 160~520 을 비율을 눌러 채우지 않는다"는 제약을 지키려고 Task 2 구현자가 내린 판단이고,
// 사후 리뷰에서 지오메트리는 승인됐다(균등 확대가 수식 구조상 강제됨 · FOOT 이탈 0).
//
//   x 160~520 을 신발의 bounding box 가 아니라 전체 모션 필드로 봤다. 신발이 다 채우지 않고,
//   남는 좌우는 여백으로 둔다.
//     x 160~190  좌측 여백 — 스타트 라인·속도선 후보 자리
//     x 190~490  러닝화 실루엣 · 발 윤곽 · 적합 영역
//     x 490~520  우측 여백 — 전진 방향 표시·앞꿈치 계측선 후보 자리
//
//   ⚠️ 좌우 여백을 실제로 채우는 주체는 이 실루엣이 아니라 KineticHero(Task 11)다.
//   스펙 §4.2(러닝 에너지 — 사선 트랙)에 스타트 라인과 속도선 3줄이 정의돼 있다.
//   위 구간 라벨은 그 요소들의 **배치 후보**이지 확정 배치가 아니다.
//
//   신발은 **원본 비율을 유지한 균등 확대만** 한다. 비균등 scale·가로 늘이기·
//   세로 누르기는 금지다(비율 왜곡이 러닝화 인지성과 중립성을 훼손한다).
//   시각 중심은 필드 중앙(340)보다 오른쪽으로 옮겨 전진감을 만든다.
//
// 접지선(baseY)은 305 에 고정한다. 확대분은 위로만 자란다 — 신발이 서 있는
// 지면이 흔들리면 트랙·계측선을 나중에 얹을 때 어긋나기 때문이다.
const FIELD = { x0: 160, x1: 520 }; // 전체 모션 필드
const SHOE_ZONE = { x0: 190, x1: 490 }; // 신발 실루엣 영역(폭 300)
// 폭 294: 실루엣 영역(폭 300)에 넣으면서 우측 이동 +5px 을 함께 주면 오른쪽이 영역을 넘는다.
// 두 조건을 다 만족시킬 수 없어 초과폭이 2px 에 그치는 값으로 골랐다(270 대비 1.089배).
// 결과로 나오는 필드 점유 81.7% 는 목표로 삼은 값이 아니라 산출값이다.
const SHOE_WIDTH = 294;
const FORWARD_SHIFT = 5; // 필드 중앙 대비 우측 이동(전진감) — 취향 판단
...

{
  ...
  // 모션 필드 3구간을 그려 실루엣이 어디에 앉는지, 좌우에 얼마가 남는지 대조한다.
  // 여백에 무엇이 들어갈지는 KineticHero(Task 11)가 정한다 — 여기서는 폭만 보여준다.
  const zones = () => {
    ...
    return (
      band(FIELD.x0, SHOE_ZONE.x0, '좌측 여백(후보)', 'rgba(255,77,0,.07)') +
      band(SHOE_ZONE.x0, SHOE_ZONE.x1, '실루엣 영역', 'rgba(23,21,15,.035)') +
      band(SHOE_ZONE.x1, FIELD.x1, '우측 여백(후보)', 'rgba(255,77,0,.07)') +
