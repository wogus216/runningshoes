...
function categoryEntries(): MatrixEntry[] {
  const angles: Record<string, { intro: string; faq: FaqItem[] }> = {
    ...
    트레일: {
      intro:
        '트레일 러닝화는 흙길·자갈·바위·진흙 같은 비포장 노면을 위한 신발입니다. 로드화와 가장 큰 차이는 ① 깊은 러그(접지력), ② 록 플레이트(돌·바위로부터 발바닥 보호), ③ 단단한 어퍼·토 범퍼입니다. 한국 산악 트레일은 북한산·관악산처럼 화강암 슬랩과 급경사 암릉이 많아 무엇보다 그립이 중요합니다. 아래는 RunRepeat 랩 데이터와 한국 산악 적용성을 함께 반영한 트레일화 추천입니다.',
      faq: [
        {
          q: '로드 러닝화로 트레일을 뛰어도 되나요?',
          a: '대모산·청계산 둘레길 같은 완만한 임도는 가능하지만, 북한산·관악산처럼 화강암과 급경사가 있는 정식 트레일에서는 위험합니다. 로드화는 그립 패턴이 부드러워 젖은 바위에서 미끄러지고, 록 플레이트가 없어 돌을 밟을 때 발바닥 보호가 안 됩니다.',
        },
        {
          q: '트레일 입문인데 카본 트레일화부터 사도 되나요?',
          a: '권장하지 않습니다. 카본 트레일화는 평탄·완만 코스에 특화돼 있고, 테크니컬 지형에서는 오히려 불안정합니다. 입문은 그립과 발 보호가 좋은 일반 트레일화(예: 가성비 모델)로 시작해 기술과 발목 근력을 먼저 다지세요.',
        },
        {
          q: '발볼이 넓은데 트레일화도 와이드 옵션이 있나요?',
          a: '트레일화는 로드화보다 와이드 옵션이 적습니다. 토박스가 넓은 모델(본문 토박스 수치 참고)을 고르거나, 와이드(2E/4E)를 제공하는 모델을 우선하세요. 발볼 압박은 다운힐에서 발가락 통증으로 이어지므로 트레일에서 특히 중요합니다.',
        },
      ],
    },
  };
