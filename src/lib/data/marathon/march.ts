import type { MarathonEvent } from '@/types/marathon';

export const marchEvents: MarathonEvent[] = [
  {
    id: 'gumi-marathon-2026',
    name: '2026 구미 박정희 마라톤',
    date: '2026-03-01',
    month: '3월',
    location: '경북 낙동강체육공원',
    region: '경북',
    distances: ['풀코스', '하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'run-your-way-half-2026',
    name: '2026 Run your way HALF',
    date: '2026-03-01',
    month: '3월',
    location: '서울 광화문광장',
    region: '서울',
    distances: ['하프'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'busan-beach-ultra-2026',
    name: '제21회 부산 비치울트라 마라톤',
    date: '2026-03-07',
    month: '3월',
    location: '부산 다대포 해수욕장',
    region: '부산',
    distances: ['울트라'],
    status: '대회종료',
    isMajor: false,
    description: '50km 울트라마라톤',
  },
  {
    id: 'yeouido-cherry-blossom-2026',
    name: '제16회 여의도 벚꽃 마라톤',
    date: '2026-03-08',
    month: '3월',
    location: '서울 여의도',
    region: '서울',
    distances: ['하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'goyang-half-2026',
    name: '2026 고양특례시 하프마라톤',
    date: '2026-03-08',
    month: '3월',
    location: '고양종합운동장 (경기 고양)',
    region: '경기',
    distances: ['하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
    website: 'https://www.goyangmarathon.com',
    description:
      '고양시체육회·중앙일보·JTBC가 공동 주최하는 봄 하프마라톤(주관 러너블). 고양종합운동장을 출발해 일산신도시 일대(경의로·호수로)를 도는 평지 코스로 약 1.5만 명 규모입니다. 2026년 대회는 3/8(일) 종료. JTBC 서울마라톤(가을)과 함께 데상트가 후원하는 JTBC 러닝 시리즈로, 봄에 한 번만 열립니다.',
    courseInfo: {
      terrain: '평지',
      difficulty: '초보자',
      courseDescription:
        '고양종합운동장 출발·도착, 일산신도시 경의로·섬말다리·호수로를 잇는 평지 코스. 하프·10K·5K로 가족 참가에도 적합합니다.',
    },
    raceInfo: {
      entryFees: [
        { distance: '하프', fee: 70000 },
        { distance: '10K', fee: 60000 },
        { distance: '5K', fee: 40000 },
      ],
      expectedParticipants: 15000,
      startTime: '08:00',
    },
  },
  {
    id: 'mbn-bluerace-geoje-2026',
    name: '2026 MBN 블루레이스 거제',
    date: '2026-03-08',
    month: '3월',
    location: '경남 거제',
    region: '경남',
    distances: ['하프', '10K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'seoul-marathon-2026',
    name: '2026 서울마라톤',
    date: '2026-03-15',
    month: '3월',
    location: '서울 광화문광장',
    region: '서울',
    distances: ['풀코스', '10K'],
    status: '대회종료',
    isMajor: true,
    website: 'https://seoul-marathon.com',
    description: '국내 최대 규모 마라톤 대회. 동아일보·서울시 주최.',
    courseInfo: {
      terrain: '평지',
      difficulty: '중급',
      elevationGain: 60,
      courseDescription: '광화문광장 출발 → 세종대로 → 한강변 → 잠실종합운동장 피니시',
      certification: 'IAAF',
      isLoopCourse: false,
    },
    raceInfo: {
      entryFees: [
        { distance: '풀코스', fee: 100000 },
        { distance: '10K', fee: 80000 },
      ],
      timeLimits: [
        { distance: '풀코스', hours: 5, minutes: 0 },
        { distance: '10K', hours: 1, minutes: 30 },
      ],
      aidStations: 8,
      aidInterval: '약 5km마다',
      expectedParticipants: 30000,
      startTime: '08:00',
    },
    logistics: {
      parking: '대회 당일 광화문 일대 교통통제, 대중교통 이용 권장',
      publicTransport: '지하철 5호선 광화문역 2번 출구',
      bagStorage: true,
    },
  },
  {
    id: 'naepo-marathon-2026',
    name: '2026 내포마라톤',
    date: '2026-03-21',
    month: '3월',
    location: '충남 내포신도시',
    region: '충남',
    distances: ['하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'jeongeup-donghak-2026',
    name: '2026 정읍동학마라톤',
    date: '2026-03-22',
    month: '3월',
    location: '전북 정읍',
    region: '전북',
    distances: ['풀코스', '하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'incheon-half-2026',
    name: '제26회 인천국제하프마라톤',
    date: '2026-03-22',
    month: '3월',
    location: '인천 문학경기장',
    region: '인천',
    distances: ['하프', '10K', '5K'],
    status: '대회종료',
    isMajor: true,
    website: 'http://www.incheonmarathon.co.kr',
    description: '2001년 창설된 26년 역사의 봄 대표 하프. WA 공인 코스(문학경기장~송도 해안 반환), 선착순 1.5만 명. 동아마라톤 직후 봄 첫 하프. 가을 인천마라톤(풀코스)과 별개.',
  },
  {
    id: 'seoul-k-marathon-2026',
    name: '2026 서울 K-마라톤',
    date: '2026-03-22',
    month: '3월',
    location: '서울 광화문광장',
    region: '서울',
    distances: ['하프', '10K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'taehwagang-marathon-2026',
    name: '제23회 태화강 마라톤',
    date: '2026-03-28',
    month: '3월',
    location: '울산 태화강 국가정원',
    region: '울산',
    distances: ['풀코스', '하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'geumgang-ultra-2026',
    name: '2026 금강울트라마라톤',
    date: '2026-03-28',
    month: '3월',
    location: '세종 금강보행교',
    region: '세종',
    distances: ['울트라'],
    status: '대회종료',
    isMajor: false,
    description: '100K/50K 울트라마라톤',
  },
  {
    id: 'gimpo-hangang-2026',
    name: '제14회 김포한강마라톤',
    date: '2026-03-29',
    month: '3월',
    location: '경기 김포',
    region: '경기',
    distances: ['풀코스', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'hapcheon-cherry-blossom-2026',
    name: '제25회 합천벚꽃마라톤',
    date: '2026-03-29',
    month: '3월',
    location: '경남 합천',
    region: '경남',
    distances: ['풀코스', '하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  {
    id: 'hangang-cherry-blossom-2026',
    name: '제2회 한강 벚꽃 마라톤',
    date: '2026-03-29',
    month: '3월',
    location: '서울 월드컵공원',
    region: '서울',
    distances: ['하프', '10K', '5K'],
    status: '대회종료',
    isMajor: false,
  },
  // --- 2027 시즌 (해외) ---
  // 참가료를 entryFees에 넣지 않는 이유는 february.ts 주석 참고(JSON-LD가 KRW 하드코딩).
  {
    id: 'tokyo-marathon-2027',
    name: '도쿄 마라톤 2027',
    date: '2027-03-07',
    month: '3월',
    location: '도쿄도청 출발 → 도쿄역 앞 교코도리 도착',
    region: '도쿄',
    distances: ['풀코스'],
    status: '마감',
    isMajor: true,
    website: 'https://www.marathon.tokyo/',
    registrationNote:
      '접수 종료 — 일반 접수(General Entry)는 8월 28일(금) 17시(JST)에 마감됐습니다. 당락은 9월 18일(금) 발표되고 참가비는 9월 18일~30일에 납부해야 하며, 이 기간을 넘기면 당첨이 취소됩니다. 해외 ONE TOKYO GLOBAL 회원 접수는 이보다 이른 8월 13일에 이미 닫혔습니다(공식 2026-08-31 확인)',
    description:
      '제20회 기념대회로 정원이 40,000명으로 늘었고 10.7km는 마라톤에 통합돼 폐지됐다. 세계 6대 마라톤 중 유일한 아시아 대회. 한국 거주자 참가료는 230 USD(사무수수료·소비세 포함, 짐 보관 희망 시 ¥1,200 별도)이고 일반 접수는 2026년 8월 14일 11시~28일 17시(JST), 추첨 결과는 9월 18일 발표(참가비 납부 9/18~9/30). 접수 첫날 신청이 정원을 넘겨 추첨 실시가 공식 확정됐으나 선착순이 아니므로 마감 전 응모하면 조건은 동일하다. 참가자격은 대회일 기준 만 19세 이상이면서 6시간 30분 이내 완주 가능한 사람이다. 20주년 기념으로 듀오 팀(2인 1팀, 별도 이메일 신청) 부문이 신설됐고 완주 메달은 지름 100mm 프리미엄 에디션이다.',
    courseInfo: {
      terrain: '평지',
      difficulty: '중급',
      certification: 'AIMS',
      courseDescription:
        '고지대인 신주쿠 도쿄도청에서 출발해 도쿄역 방향으로 내려가는 구조라 평탄하고 빠른 코스라는 평이 정착돼 있다. 스이도바시·우에노히로코지·칸다·니혼바시·아사쿠사 카미나리몬·료고쿠·몬젠나카초·긴자·다마치·히비야를 지난다. 일본육상경기연맹·AIMS·월드 애슬레틱스 공인 코스.',
    },
    raceInfo: {
      timeLimits: [{ distance: '풀코스', hours: 7, minutes: 0 }],
      expectedParticipants: 40000,
      startTime: '09:10',
    },
    logistics: {
      bagStorage: true,
    },
    registrationStart: '2026-08-14',
    registrationEnd: '2026-08-28',
    lastVerified: '2026-08-14',
  },
];
