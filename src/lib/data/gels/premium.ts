    ...
    reviews: [
      {
        userType: '서브3 마라토너',
        rating: 5,
        text: '서울마라톤 서브3 도전 때 모텐으로 바꾸고 나서 30km 이후 복통이 완전히 사라졌습니다. 25g 탄수화물에 100kcal로 가볍고, 하이드로젤 덕분에 물 없이 삼켜도 위에 부담이 없어요. 맛은 밋밋하지만 레이스에서는 맛보다 속이 편한 게 진리입니다. 20분마다 1개씩 5개를 먹어도 속이 편한 건 모텐뿐이었어요. 개당 6,400원이 아깝지 않은 이유가 있습니다.',
      },
      {
        userType: '위장 예민 러너',
        rating: 5,
        text: '경주 벚꽃마라톤에서 다른 젤 2개 먹고 25km 지점에서 화장실 달려간 경험이 트라우마였는데, 모텐은 4개까지 먹어도 속이 전혀 안 뒤집어집니다. 카페인 0mg에 나트륨 28mg만 들어있어서 위장에 자극이 거의 없어요. 개당 6,400원, 풀코스 5개면 3만 2천 원이라 부담되지만 DNF 방지비용이라 생각합니다. DC 러닝갤에서도 "위장 약하면 모텐 가라"가 정설이더라고요.',
      },
      {
        userType: '가성비 러너',
        rating: 3,
        text: '솔직히 개당 6,400원짜리 젤이 800원짜리 양갱보다 8배나 좋은지 모르겠습니다. 100kcal에 탄수화물 25g이면 양갱이랑 비슷한 수준인데 가격은 하늘과 땅 차이. 춘천마라톤 때 써봤는데 위장이 튼튼한 저한테는 GU Roctane(3,500원)이면 충분했어요. 위장이 정말 예민한 분들한테는 추천하지만, 그게 아니면 가성비 젤부터 써보세요.',
      },
    ],
    ...
    reviews: [
      {
        userType: '시리어스 마라토너',
        rating: 4,
        text: '동아마라톤 서브3:30 도전 때 Beta Fuel로 전환했습니다. 서빙당 40g 탄수화물에 158kcal이라 3개만 먹으면 120g 확보되니까 보충 횟수가 확 줄어요. 등장(Isotonic) 젤이라 물 없이 삼킬 수 있는 것도 급수대 타이밍 안 맞을 때 큰 장점이에요. 나트륨 40mg에 칼륨 20mg까지 들어있어서 전해질 보충도 동시에 됩니다. 다만 60ml 패킷이 커서 러닝벨트 주머니에 3개 넣으면 좀 불편합니다.',
      },
      {
        userType: 'LSD 훈련 러너',
        rating: 4,
        text: '한강 뚝섬~잠실 구간 30km LSD 훈련 때 주로 사용하고 있습니다. 오렌지 맛이 상큼해서 25km 넘어서도 거부감이 없어요. 처음에는 40g 고농축이라 속이 좀 더부룩했는데 장 훈련 2주 후부터 확실히 편해졌습니다. GU 대비 목넘김이 훨씬 부드럽고, 개당 4,400원(벌크 3,100원)으로 모텐(6,400원)보다 합리적입니다. 비타민 B6/B12까지 포함이라 장거리 후 피로 회복에도 도움되는 느낌.',
      },
      {
        userType: '철인3종 선수',
        rating: 4,
        text: '통영 트라이애슬론 때 Beta Fuel만으로 레이스 전체를 커버했습니다. 나트륨·칼륨·마그네슘 전해질이 다 들어있어서 별도 전해질 캡슐을 안 챙겨도 되는 게 올인원의 최대 장점이에요. 사이클 구간에서 60ml 한 번에 짜먹기 딱 좋고, 러닝 구간에서도 등장이라 물 없이 OK. 4가지 맛 로테이션하면 맛 피로감도 없습니다. 마라톤만 하시는 분은 패킷 크기가 부담일 수 있지만 멀티 스포츠에는 최고.',
      },
    ],
  ...
  },
  {
    ...
    reviews: [
      ...
      },
      {
        userType: '사이클리스트',
        rating: 5,
        text: '북한강 라이딩 때 226ERS가 메인 연료입니다. 사이클로덱스트린 기반이라 100km 넘게 타도 속이 편하고, 9가지 맛을 로테이션하면 맛 피로감이 제로예요. 특히 에스프레소(카페인 80mg)와 체리(카페인 160mg) 라인업이 있어서 후반부에 체리를 전략적으로 투입하면 확실히 각성 효과가 있습니다. 쾰른 리스트 도핑 방지 인증까지 받아서 아마추어 대회 출전하는 분들도 안심하고 쓸 수 있어요. 자전거 저지 주머니에는 76g 패킷도 문제없이 들어갑니다.',
      },
    ],
