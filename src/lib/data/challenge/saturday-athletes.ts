...
export const ATHLETES_COPY = {
  ...
  //
  // 효과를 설명하지 않고 부탁만 한다. '한 명씩 보인다'는 거짓이고(한 번에 한 명만
  // 넘어가는 건 스냅 클램프가 이미 보장한다 — use-deck-scroll.ts), '잘 보인다'도
  // 무엇이 잘 보이는지 말하지 않는 빈 약속이었다. 천천히 내려서 실제로 달라지는 것은
  // 사진이 아래에서 올라오는 그 과정을 보느냐뿐이라, 그건 안내문이 설명할 게 아니라
  // 내려보면 아는 것이다.
  scrollAdvice: '천천히 내려주세요',
