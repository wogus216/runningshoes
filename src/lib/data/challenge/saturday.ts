...
export const SATURDAY_COPY: SaturdayTeaserCopy = {
  hero: {
    ...
    turn: '웃자고 한 말이었다.',
    // 2026-08-19 에 단톡 원문('신청 완료했습니다!!')을 그대로 올렸다가 되돌렸다(2026-08-20).
    // 이유: 히어로 나머지 세 줄이 전부 서술문인데 한 줄만 말풍선 없는 인용이라
    // 화자가 없어서 시스템 메시지처럼 읽혔다. 원문 존중은 단톡 섹션이 하는 일이고,
    // 히어로는 그 앞에 서는 설명이다 — 여기서는 사실을 쓰는 게 맞다.
    // '일곱 명'은 SATURDAY_CREW 길이와 일치해야 한다(아래 crew.title 과 같은 규칙).
    hook: '일곱 명이 신청했다.',
  ...
  crew: {
    ...
    // 운영자 확인(2026-08-20). 이게 없으면 "왜 하필 이 일곱 명인가"에 답이 없다 —
    // 회사 동료도 러닝크루도 아니고 중학교 때 만난 사이라는 게 이 이야기의 전제다.
    bond: '중학교 때 만난 친구들',
  },
  race: {
  ...
  footer: {
    // 운영자 본인 말이다. 맞춤법·띄어쓰기만 정리했다(단톡 3원칙과 같은 기준).
    // 원문: "맨처음 토요일 아침에 주로 띄어서 ㅋㅋㅋㅋ쎄러데이라고 지었고
    //        그냥 썬데이보다는 어감이 신나서 그렇게 지었어"
    //
    // 이 줄이 없으면 바로 아래 joke 가 뜬금없다 — 이름이 토요일에서 왔다는 걸
    // 이미 안다고 전제하고 던지는 농담이기 때문이다(실사용 피드백으로 확인).
    // 2026-08-20 문장을 닫았다 — 앞판('…뛰어서 쎄러데이. 썬데이보다 어감이 신나서.')은
    // 토막 둘이라 말이 끊긴 것처럼 읽혔다. 어휘는 그대로 두고 어미만 푸터 말투(합니다체)에 맞췄다.
    nameOrigin: '처음엔 토요일 아침에 주로 뛰어서 쎄러데이라고 지었습니다. 썬데이보다 어감이 신나서요.',
    // 썬데이를 어감 때문에 버렸는데, 위 단톡에는 일요일 아침에 뛰기로 하는 장면이 있다.
    // 설명하지 않는다 — 앞을 읽은 사람에게만 걸리면 된다.
    joke: '토요일에만 뛰는 건 아닙니다.',
  },
};
