...
export const shoe: Shoe = {
  ...
  editorComment: 'RunRepeat Lab 역대 최고 에너지 리턴 80.6%라는 수치는 알파플라이 3(78.2%)도 넘어서는 기록입니다. IncrediRUN 폼의 6.5 HA 극소프트 경도가 만들어내는 반발력은 경이적이지만, 동시에 안정성 4/10이라는 대가를 치릅니다. 33만 9천원이라는 가격과 200km 내구성은 km당 약 1,695원을 의미하며, 이건 오직 레이스데이에만 꺼내 신는 무기입니다. 월 200km 이상 훈련하는 서브3 러너가 아니면 이 신발의 잠재력을 끌어내기 어렵고, 토박스가 좁아 한국 러너 발볼과의 궁합은 도전적입니다. IncrediRUN은 TPEE 계열이라 순수 PEBA보다 마모 내성 면에서 유리한 소재 선택입니다 — 그래도 레이스 전용 설계인 만큼, 훈련 마일리지는 데일리에 맡기는 운용이 기본입니다.\n\n사이즈·구매 가이드: 토박스가 좁은 편(narrow)이므로 발볼이 넓다면 와이드 옵션이 없어 착용이 어려울 수 있으니 반드시 시착하세요. 정가 339,000원에 내구성 200km 기준 km당 약 1,695원으로 레이스 전용 운용이 필수입니다. 같은 엘리트 레이싱화로는 알파플라이 3·메타스피드 엣지+가 대안입니다.',
  ...
  reviews: [
    ...
    {
        userType: '예상 적합 — 후반 반발 유지가 과제인 마라토너',
        text: '30km 이후 반발이 죽는 것이 과제인 러너를 겨냥한 최상위 레이서입니다. 에너지 리턴 80.6%는 시판 레이서 중 최상단이고, IncrediRUN 폼이라 후반까지 탄성이 유지되는 성격입니다. 197g의 무게에 HA 6.5의 소프트 폼 조합이라, 반발이 강하면서도 착지 충격은 흡수하는 드문 구성입니다. 다만 토박스가 좁아 발볼이 넓다면 맞지 않습니다.',
      },
    {
        userType: '비적합 — 서브3:30 전후 러너',
        text: '이 모델의 성능 차이는 매우 빠른 페이스에서만 드러납니다. 서브3:30 안팎이라면 같은 라인의 프로 4로도 충분하고, 엘리트 2와의 차이를 체감하려면 3:30/km 수준의 페이스가 필요합니다. 안정성이 4/10으로 낮아 폼이 완성되지 않은 단계에서는 착지가 불안해질 수 있고, 토박스가 좁아 발볼이 넓으면 15km 이후 통증이 생깁니다. 339,000원·내구성 200km 기준 km당 약 1,700원이라, 성능을 못 쓰는 상태에서 부담만 큰 선택이 됩니다.',
      },
    {
        userType: '적합성 경계 — 반발은 최상, 발볼은 최악',
        text: '성능과 핏이 정반대로 갈리는 모델입니다. 반발력만 놓고 보면 최상위 레이서 3파전(알파플라이·메타스피드 스카이+·엔돌핀 엘리트 2) 중 앞서는 편으로, 에너지 리턴 80.6%에 HA 6.5의 극소프트 폼인데도 발이 빠지지 않고 튕겨주는 감각입니다. 반대로 토박스는 이 등급에서도 가장 좁은 축이라, 발볼이 넓은 편이면 시착 없이 구매하는 것은 위험합니다. km당 약 1,695원으로 단가도 높아 대회 전용 운용이 전제입니다.',
      },
  ],
