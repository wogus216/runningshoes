...
export const posts_2025_01: BlogPost[] = [
  {
    id: 'wide-feet-korea',
...
<h3>발볼이 안 맞으면 왜 문제가 되나</h3>
<p>
  발볼 문제의 까다로운 점은 <strong>매장에서는 잘 드러나지 않는다</strong>는 것입니다.
  가만히 서 있을 때는 여유가 있어 보여도, 달리기 시작하면 착지할 때마다 발이 앞뒤로 밀리고
  발바닥이 눌리면서 옆으로 퍼집니다. 장거리에서는 혈류가 늘어 발 자체가 붓기까지 합니다.
  매장에서 괜찮았던 신발이 10km 지점에서 조이는 이유가 여기 있습니다.
</p>
<p>
  흔한 대응이 반 사이즈를 올리는 것인데, 이건 <strong>폭 문제를 길이로 푸는 방식</strong>이라
  대가가 따릅니다. 앞볼은 여유가 생기지만 뒤꿈치가 헐거워져 발이 신발 안에서 미끄러지고,
  그 미끄러짐이 물집이나 아킬레스 자극으로 이어집니다. 폭이 문제라면 길이가 아니라
  <strong>와이드 라스트</strong>로 풀어야 하는 이유입니다.
</p>
<p>
  압박을 방치하면 불편함에서 끝나지 않습니다. 발톱이 반복해서 눌리면 검게 변하고(러너 발톱),
  발가락 사이가 습해지면 진균 감염 위험이 올라갑니다. 앞발가락뼈 사이의 신경이 오래 눌리면
  저림이나 화끈거림이 나타나기도 합니다. 러닝을 계속하려면 발이 아프지 않은 게 먼저입니다.
</p>
<p>
  그래서 이 글은 "평이 좋은 신발"이 아니라 <strong>토박스 실측 폭</strong>을 기준으로 정리했습니다.
  아무리 리뷰가 좋아도 내 발볼에 맞지 않으면 의미가 없기 때문입니다.
</p>

<div class="callout info">
  <span class="callout-icon">ℹ️</span>
  <div class="callout-body">
    <p class="callout-title">핵심 요약</p>
...
<p>
  ...
  DNA LOFT 폼이 부드러운 편이고, Wide 버전은 표준 대비 앞볼 폭이 실제로 넓어집니다.
</p>
...
<p>
  발볼이 넓은 러너가 신발을 고를 때 반복해서 빠지는 함정들입니다.
...
<p>
  "발볼이 넓어서 신을 신발이 없다"는 말은 이제 사실이 아닙니다.
  와이드 라스트를 제대로 내는 브랜드가 어디인지만 알면 선택은 오히려 명확해집니다.
  이 글이 자신에게 맞는 신발을 찾는 출발점이 되기를 바랍니다.
</p>
<p>
...
<h3>무릎 통증이 생기는 전형적인 경로</h3>
<p>
  러너의 무릎 통증은 대개 한 번의 사고가 아니라 <strong>누적으로</strong> 옵니다.
  가장 흔한 경로가 훈련량을 급격히 늘리는 것입니다. 주간 거리를 30km에서 50km로
  한 번에 올리면 근육과 힘줄이 적응할 시간을 못 얻고, 2주쯤 지나 무릎 바깥이나 안쪽이
  뻐근해지기 시작합니다. 주간 증가폭을 10% 안팎으로 두라는 오래된 조언이 여기서 나옵니다.
</p>
<p>
  이 단계에서 "며칠 쉬면 낫겠지" 하고 계속 달리면 문제가 커집니다. 계단을 내려갈 때
  찌릿한 통증이 나타나는 시점이면 이미 조직에 부하가 쌓인 상태입니다. 러너에게 흔한
  <strong>장경인대증후군(ITBS)</strong>이나 <strong>슬개대퇴통증증후군</strong>이 이 무렵 진단되는데,
  원인으로 지목되는 것이 대체로 급격한 운동량 증가와 부적절한 신발 두 가지입니다.
</p>
<p>
  신발 쪽 함정은 <strong>용도가 안 맞는 신발로 거리를 쌓는 것</strong>입니다. 가볍고 얇은 템포화는
  빠른 훈련에는 맞지만 장거리를 매일 소화하기에는 충격 흡수가 부족합니다. 거리를 늘리는
  시기라면 쿠셔닝과 안정성이 확보된 신발로 바꾸고, 복귀할 때도 주간 거리를 천천히 올리는 편이
  안전합니다.
</p>
<p>무릎 통증의 주요 원인:</p>
<ul>
...
<p>
  ...
  회내를 잡아주는 구조라 무릎 안쪽으로 쏠리는 부하를 줄이는 방향으로 설계됐습니다.
</p>
