...
export function ReplacementCalculator({ durability, shoeName }: ReplacementCalculatorProps) {
  ...
  return (
    <section className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="rounded-xl border border-border bg-surface/50 p-3">
            ...
            <div className="mt-1 text-xl font-black text-slate-950">
              {result.minMonths}~{result.maxMonths}<span className="text-sm">개월</span>
            </div>
          </div>
          <div className="rounded-xl border border-border bg-surface/50 p-3">
            <div className="text-[10px] uppercase tracking-wider text-tertiary font-semibold">주</div>
            <div className="mt-1 text-xl font-black text-slate-950">
              {result.minWeeks}~{result.maxWeeks}<span className="text-sm">주</span>
            </div>
          </div>
          <div className="rounded-xl border border-border bg-surface/50 p-3">
            <div className="text-[10px] uppercase tracking-wider text-tertiary font-semibold">누적 km</div>
            <div className="mt-1 text-xl font-black text-slate-950">{min}~{max}</div>
          </div>
        </div>
      )}
      ...
      {durability && (
        <div className="rounded-2xl border border-border bg-surface/40 p-4 text-xs leading-relaxed text-secondary">
