...
function ComparePageContent({ allShoesProp }: { allShoesProp: CardShoe[] }) {
  ...
  const [draggedShoe, setDraggedShoe] = useState<CardShoe | null>(null);
  ...
  useEffect(() => {
    ...
    if (shoesParam) {
      ...
      const loadedShoes = slugs
        .map(slug => allShoes.find(s => s.slug === slug))
        .filter((shoe): shoe is CardShoe => shoe !== undefined && shoe.specs !== undefined);
  ...
  const addShoeToSlot = (shoe: CardShoe, slotIndex: number) => {
