...
- [ ] seoul-race-2026 (october.ts) **status 검증 필요**: DB는 `접수중`인데 본접수가 5/27이라 8월 현재 열려 있을 가능성이 낮음. 2026-08-12 확인 시도했으나 공식(seoul-race.co.kr) WebFetch 403 + curl 무응답으로 실패 → **날짜 경과만으로 마감 단정 금지 원칙에 따라 DB 미변경**. Chrome MCP 등 접근 가능한 경로로 재확인 후 `마감` 여부 판정. 비교글(`october-fun-run-races-2026-compare`)엔 "잔여 여부 공식 확인 필요"로 보수 표기해둠 (추가 2026-08-12)
