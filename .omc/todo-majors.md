...
- [x] **HUB 세계 6대 마라톤 완전정복** (`world-marathon-majors-guide-2026`, 2026-06-13 발행) — 비교표·의사결정 트리·난이도순 + **9개 글 전부 링크** + FAQPage JSON-LD(5문). 시리즈 관문 완성

## ✅ 시리즈 완성 (10/10, 2026-06-13)
- 발행 완료: 도쿄·시드니·시카고·보스턴·뉴욕·런던·베를린(스포크 7) + 해외원정매뉴얼·식스스타(보조 2) + 허브 1
- 내부링크: 허브→9개 전부, 식스스타→스포크 전부, B1→5도시, 스포크↔형제. 강한 그물
- **남은 선택 작업(에버그린 강화)**: 각 스포크 본문에 허브 백링크 1줄 추가(spoke→pillar)로 SEO 양방향 완성 — 시간 날 때
- 시의성 리뉴얼: 월간 routine `trig_01MVVHXVgqL3YmooHCn1iZyT` + 날짜 트리거 + EU ETIAS 재검증이 10개 글 자동 감시
